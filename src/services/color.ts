...
export class Solver {
    ...
    spsa(A: number, a: Array<number>, c: number, values: Array<number>, iters: number): {
        values: Array<number>,
        loss: number,
    } {
        const alpha = 1;
        ...
        let best: Array<number> | null = null;
    ...
    loss(filters: Array<number>): number {
    ...
    raw(filters: Array<number>): string {
    ...
    css(filters: Array<number>): string {
        function fmt(idx: number, multiplier = 1): number {
            return Math.round(filters[idx] * multiplier);
        }
